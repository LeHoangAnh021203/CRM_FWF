...
function isRateLimited(identifier: string, isApi: boolean = false): boolean {
  const now = Date.now();
  const config = isApi ? 
    { window: CONFIG.RATE_LIMIT.API_WINDOW_MS, max: CONFIG.RATE_LIMIT.API_MAX_REQUESTS } :
    { window: CONFIG.RATE_LIMIT.WINDOW_MS, max: CONFIG.RATE_LIMIT.MAX_REQUESTS };
...
function isBlockedRequest(request: NextRequest): boolean {
  ...
  if (CONFIG.SECURITY.BLOCKED_USER_AGENTS.some(blocked => 
...
function checkAuthentication(request: NextRequest): { isAuthenticated: boolean; token: string | null; userRole: string | null } {
...
export default function middleware(request: NextRequest) {
  ...
  console.log(`[Middleware] ${method} ${pathname} - ${new Date().toISOString()}`);
  ...
  if (!pathname.startsWith('/forgotPassword') && isRateLimited(rateLimitKey, isApiRoute)) {
  ...
  const isPublicRoute = CONFIG.PUBLIC_ROUTES.some(route => pathname.startsWith(route));
  const isAdminRoute = CONFIG.ADMIN_ROUTES.some(route => pathname.startsWith(route));
  ...
  const isMediaFile = /\.(mp4|webm|ogg|jpg|jpeg|png|gif|svg|ico|css|js|woff|woff2|ttf|eot)$/i.test(pathname);
  
  // If user is authenticated, allow access to all routes
  if (isAuthenticated) {
    console.log(`[Middleware] Authenticated user accessing ${pathname}`);
  } else if (!isPublicRoute && !isMediaFile && !pathname.startsWith('/dashboard') && !pathname.startsWith('/forgotPassword')) {
  ...
  if (!pathname.startsWith('/forgotPassword') && isAdminRoute && userRole !== 'ROLE_ADMIN') {
    console.log(`[Middleware] Unauthorized admin access: ${userRole} to ${pathname}`);
    return new NextResponse('Forbidden', { status: 403 });
  }
  ...
  if (!pathname.startsWith(`/${preferredLocale}`) && 
    ...
    console.log(`[Middleware] Localization redirect: ${pathname} -> ${localizedUrl.pathname}`);
  ...
  if (CONFIG.AB_TESTING.ENABLED && !pathname.startsWith('/forgotPassword')) {
    ...
    if (pathname === '/' || pathname === '/homepage') {
      ...
      console.log(`[Middleware] A/B Test - Homepage variant: ${variant} for user: ${userId}`);
    }
    ...
    if (pathname.startsWith('/pricing')) {
      ...
      console.log(`[Middleware] A/B Test - Pricing variant: ${variant} for user: ${userId}`);
    }
  }
  ...
  console.log(`[Middleware] Completed ${method} ${pathname} in ${processingTime}ms - Auth: ${isAuthenticated}, Role: ${userRole}, Country: ${userLocation.country}`);
