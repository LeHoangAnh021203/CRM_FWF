...
export function RejectModal({ shift, onReject, onCancel }: RejectModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    const finalReason = selectedReason === "Khác (ghi rõ lý do)" ? reason : selectedReason || reason
    if (finalReason.trim()) {
        ...
        <CardContent>
          <div className="mb-4 p-3 bg-gray-50 rounded-lg">
            ...
            <div className="text-sm text-gray-600 space-y-1">
              ...
              <p>
                <strong>Ngày:</strong> {new Date(shift.date).toLocaleDateString("vi-VN")}
              </p>
              <p>
            ...
            {(selectedReason === "Khác (ghi rõ lý do)" || !selectedReason) && (
              <div>
                <Label htmlFor="customReason">
                  {selectedReason === "Khác (ghi rõ lý do)" ? "Lý do cụ thể" : "Hoặc nhập lý do tùy chỉnh"}
                </Label>
                <Textarea
                  id="customReason"
                  value={reason}
                  ...
                  rows={3}
                  required={selectedReason === "Khác (ghi rõ lý do)" || !selectedReason}
                />
              </div>
            )}
